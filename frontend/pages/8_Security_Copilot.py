...
from copilot import ask_copilot, is_ollama_available, DEFAULT_MODEL
...
def render_findings(result: dict, idx: int):
    ...
    narrative = result.get("narrative") or result.get("ai_narrative", "")
    provider = result.get("llm_provider", "rule-based")
    provider_label = "Qwen (local RAG)" if provider == "qwen-local" else "Rule-based fallback"
...
def process_query(question: str):
    ...
    result = ask_copilot(question, df, use_llm=OLLAMA_OK)
    st.session_state.copilot_messages.append(
        {
            "role": "assistant",
            "content": result.get("narrative", result.get("summary", "")),
            "result": result,
        }
    )
...
st.markdown("---")
st.caption("TrustGuardian Security Copilot • Pandas RAG retrieval • Local Qwen via Ollama")
